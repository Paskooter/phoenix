...
store), `media` (the app's Gallery store, self-hosted objects), plus build-to-spec stubs for
`rom`/`person`/`ifttt`/`nlp`/`collision` (`packages/classic`). Not built (no client API contract in
the archive): `voicetraining`, `jot`. The conversational stack (hub/parser/skills) is Phoenix's
main body and is separate from these.
...
| **person** | `person-2016-08-01` | `jiborobot/srv-person-ws` | Person/loop/account properties | **🟡 stub** | Real in-memory property round-trip + holidays; exercised without the app. |
...
- **Pairing and identity**: ✅ **done** — `account` + `loop` + `oobe` in `packages/account`,
  ...
  adopt page) is the path for an already-paired robot. See
...
- **Remote control and notifications** (Commander, push, the phone app's live features):
  `entrypoint-socket` + `notification` + `key` + `robot` are **done**; `push` registers devices
  and does not deliver (there is no APNs/FCM/app to deliver to), and `rom` (Commander) is built
  to its wire contract, which is everything the server side can do without the app.
- **Features**: `media`, `backup`, `log` and `ifttt` are built; `person` and `collision` keep
  real stub behavior; `voicetraining`, `jot` and the `skill` store have no client API contract in
  the archive and are not built. `backup` is built for real because the UI's wipe/factory-reset
  gates on it (see the inventory above and [DIVERGENCES.md](DIVERGENCES.md)).
