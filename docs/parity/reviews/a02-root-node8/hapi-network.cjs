...
async function main() {
  ...
  const output = {
    ...
    networkCases: await Promise.all(cases.map(async function(entry) {
      var raw = entry[0] === 'missing-body' ? '' : entry[0] === 'top-string' ? entry[1] : JSON.stringify(entry[1]);
      return {name:entry[0], requestHeaders:{'content-type':'application/json','connection':'keep-alive'}, rawBody:raw, response:await networkProbe(server,raw)};
    })),
    cases: results,
  };
