...
The workbench at /home/shell/work/hermes-be/firmware already builds, composes, validates and packages images (Buildroot production profile -> compose-legacy-modern.js -> validate-modern-images.js -> package-ota.js), and OTA packages cut from the modern production build already exist on the resource host (root@192.168.1.23:/srv/jibo-release/ota-production). What no image carries is the repoint: the published packages are the stock 13.0.0 build, and everything that lets a robot reach Phoenix is applied after the fact over SSH. The updater's own contract decides the shape of the fix -- postinstall runs on the OLD root after the incoming filesystem has already been written and unmounted, so the repoint must be baked into the image (as stage-modern-userland.js already does for the modern userland), not performed by a hook. The repoint spans four partitions and an os/services update replaces only three: /etc/hosts and the CA-trust change live in rootfs, the jetstream hub override in services, BE under /opt in skills, and the region/server URL in /var -- which is preserved on purpose, and is exactly why a never-repointed robot would upgrade cleanly and still not find the server.
...
- Bake the repoint configuration into the image, not a postinstall hook: the hosts entry and the patched @jibo/jibo-server-client CA handling (DIVERGENCES R1) in rootfs, the /usr/local/etc/jibo-jetstream-service.json hub override in services, and BE 11.0.1 (phoenix-parity-11-0-1) under /opt/jibo/Jibo/Skills in skills. 12.0.0-era @be/be is not acceptable: it loses the cyan listening eye, the proactive runtime and the Nimbus follow-ups.
- Settle and record in DIVERGENCES how a never-repointed robot learns this server's URL, given that /var/jibo/credentials.json is preserved and not part of an os package: a default baked from the configured public URL, the download url the Update service already returns pointing at this host, or a narrowly scoped idempotent hook. Name which one, and what happens when the configured URL is unset.
- Satisfy the updater's real contract: subsystem names os and services, an uncompressed outer tar carrying ./filesystem.tar.bz2, and any hook present only if it is idempotent and provably safe, because a hook that errors is a FATAL error that triggers a redownload-and-retry loop.
...
- Validate the package offline with validate-ota.js before it is ever served, and record the image hashes and geometry alongside it.
- Falsification: inspect the BUILT tar to show the repoint is genuinely inside it rather than only in the build script, and show a robot with no manual repoint reaches the server while one pointed at a deliberately wrong URL does not.
