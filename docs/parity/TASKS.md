...
Candidate scope: Root accepted bounded photo update/removal, durable configured storage and public Classic ingress after source review, original Node8 client seven strict-TLS lifecycle checks, Docker container replacement persistence, 876 unit passes with seven skips, and strict43 on unchanged production code. Five observed synthetic test helpers close connections to avoid reproduced idle-pool races; no production timeout/retry changes. Moth deployment and whole A-04 remain open.

Candidate report: [packages/account/src/loopMemberPhotos.js](../../packages/account/src/loopMemberPhotos.js).

Lead review: Codex root; [docs/parity/evidence/2026-09-08/photo-acceptance/review.json](../../docs/parity/evidence/2026-09-08/photo-acceptance/review.json). Complete task acceptance is still governed by the main checkbox above.
