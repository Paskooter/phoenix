...
It changes only two synthetic Account test request helpers:
`packages/account/test/loopMemberUpdate.test.js` and
...
In both runs, the same-process tests performed synchronous fixture/store work
between requests. Node's `fetch` pool reused a connection after the service's
5,000 ms idle timeout. The server diagnostic recorded a `socket-timeout` on
the pooled socket at the next `fetch-start`; the mutating test then failed with
`TypeError: fetch failed`, cause `ECONNRESET`. The failure was in the local
test transport lifecycle, before a request reached the handler. No product
timeout or request retry is changed here.

## Repair

The two affected helpers now add `Connection: close` to each local test
request after signing. Closing each test connection prevents an expired
...
It exited `0` with 11 tests passed and 0 failed. The receipt is
...
| `affected-after.tap` | `bd750f9cb13c253ca5908731a4da5a8318a18a8845df82f473109a855a14e790` |
