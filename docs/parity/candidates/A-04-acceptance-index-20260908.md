...
| Duplicate and repeated invitations | Same-email duplicate, declined/removed reinvite; compare source membership identity/status, mail/event recipients and next read | Root verified the corrected source hashes and nine sequential semantic matches. Concurrent duplicate insertion remains a confirmed mismatch under repair. [Review](../evidence/2026-09-08/invitation-sequence-review/review.json). |
| Simultaneous membership transitions | Two source-reachable requests on one membership; compare allowed success/error and durable outcomes rather than inventing one deterministic order | Root confirmed source accepts both overlapping status writes; Phoenix decline-then-accept returns 200/404. Root rejected c3b after unchanged-status and shifted-member-index failures; Luna Max is correcting shared save semantics. |
| Remaining query projections | Identify an uncovered source-reachable dangling/soft-deleted reference case, compare source projection and valid follow-up; avoid redoing covered null/casting cases | Valid uppercase ObjectId repair is included in the isolated A-05 integration candidate; source casting and original Node8 client controls passed. Final combined candidate acceptance remains pending. |
...
Root concurrency review: actual Mongoose save controls and exact-controller reruns confirm array append and status-write mismatches in Phoenix. [The repair remains open](../evidence/2026-09-08/invitation-concurrency-review/review.json). The corrected nine-case sequential review does not close concurrent behavior.
