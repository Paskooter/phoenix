...
| Simultaneous membership transitions | Two source-reachable requests on one membership; compare allowed success/error and durable outcomes rather than inventing one deterministic order | Root confirmed source accepts both overlapping status writes; Phoenix decline-then-accept returns 200/404. Luna Max is implementing a bounded repair for root review. |
