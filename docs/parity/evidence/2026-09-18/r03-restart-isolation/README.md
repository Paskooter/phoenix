...
Date: 2026-09-18  
Status: **bounded measured lane passes; server-side ASR continuity remains unknown.**
...
uses a loopback LLM fixture, and shifts every contract service by `--offset 700`.
...
* Server-side microphone/Parakeet ASR continuity was not measured. The run used
  `CLIENT_NLU` turns and deliberately did not contact the machine/LAN ASR endpoint.
  No claim is made about an in-flight server-side ASR session across restart.
...
this lane; the result is an evidence boundary and identifies the ASR and publisher
