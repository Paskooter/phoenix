...
| A3 | A member account whose `photoUrl` is `null` has the field **omitted** from Phoenix's Loop member projection; source `loadMembers` copies `photoUrl` through unconditionally, so the wire carries `photoUrl: null`. | `packages/account/src/model.js` `copyAcceptedAccount` guards every field with `!= null`, which is correct for the other fields but wrong for this one. Source `loop.ctrl.ts` `loadMembers` builds its `accountCopy` with a bare `photoUrl: account.photoUrl`, with no conditional. Verified 2026-09-10 against pinned `jiborobot/srv-account-ws@6cea434 src/controllers/loop.ctrl.ts`. | **Client-visible, unlike L2.** The pinned `loop-2016-03-24` API model declares `photoUrl` on the `MemberAccount` shape, so a generated reference client *does* parse the field — a consumer can distinguish "no photo" (`null`) from "field absent". This was latent while no operation could null an existing `photoUrl`; the A-03 `RemovePhoto` slice makes it reachable, because `removePhoto` sets `photoUrl = null` on a live account that may be a loop member. Reported by the photos candidate and confirmed by root rather than accepted on the candidate's word. **Not yet repaired** — the fix is a one-line unconditional copy, but it touches the A-04 Loop projection, whose tests are accepted, so it is recorded here and queued rather than changed mid-wave. |
