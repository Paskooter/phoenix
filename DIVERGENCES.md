...
| H07b | **Parakeet applies `earlyEOS` post-hoc.** The pinned `ParakeetASRSession.ts` builds `fastEOSRegex` and then never uses it. Phoenix completes the stated intent and annotates `FAST_EOS` in `finalize()`, so the batch path does not silently drop the annotation. Intent-derived, not observed in the reference. |
