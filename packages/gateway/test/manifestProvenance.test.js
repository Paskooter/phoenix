...
test('S-06: index/manifest provenance classified per file (identical, transformed, authored)', () => {
  ...
  // Re-homed manifest trees. The frozen Pegasus target keeps the news
  // descriptor empty; requestNews is registered by report-skill in the
  // pinned source registry and must not be inferred from the later @dev
  // integration archive.
  ...
  assert.deepEqual(fixture.sourceEquivalence['external-skills'].differing, []);
