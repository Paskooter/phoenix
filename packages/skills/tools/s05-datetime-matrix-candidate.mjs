// S-05 differential probe — Phoenix candidate.
//
// Body-identical to s05-datetime-matrix-source.cjs except for the imports, so a
// diff between the two outputs isolates implementation behavior.
//
// usage: S05_CANDIDATE_ROOT=/path/to/phoenix node s05-datetime-matrix-candidate.mjs <matrix.json> <out.json>
...
for (const now of spec.nows) {
  Date.now = () => Date.parse(now);
  for (const iso of spec.seasonDates) {
    out.phrasing.push({ id: `t|${now}|${iso}`, value: safe(() => dtFields(buildPromptData({ location: { iso } }))) });
  }
}
