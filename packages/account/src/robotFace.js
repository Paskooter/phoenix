...
export function robotFaceRoutes(store, { settingsProviders = null, loopUpdatedOutbox = new LoopUpdatedOutbox(store), loopConfig = {}, agreementProvider = new EchoSignProvider(loopConfig), invitationProviders, identityProviders, robotReadClient, memberPhotoProvider } = {}) {
  ...
  const dispatch = async ({ req, res, body, log }) => {
    ...
    // Account identity core plus the email/phone/terms slice. CreateHubToken
    // stays on the bounded A-02 SigV4 path below. Unimplemented Account
    // operations keep the existing unknown-target response so Classic still
    // proxies them without a local handler.
    if (/^account/i.test(prefix)) {
      const identity = await handleAccountIdentity({
        store, req, res, body, log, identityProviders,
      });
