...
// G.2 — robot-facing AWS-JSON face: setupRobot happy path, exact error envelopes, exact unsigned targets,
// prepareRobot via verified SigV4 credentials, getStatus, Update_* proxy to a mock OTA.
...
test('setupRobot: portal-minted token -> credentials, one-time, robot adopted into a loop', async () => {
  ...
  const r = await amz('OOBE_20161026.SetupRobot', { token: token._id, id: 'castle-cylinder-fig-quilt' });
  ...
  const replay = await amz('OOBE_20161026.SetupRobot', { token: token._id, id: 'castle-cylinder-fig-quilt' });
  ...
  const again = await amz('OOBE_20161026.SetupRobot', { token: t2._id, id: 'castle-cylinder-fig-quilt' });
  ...
  const thief = await amz('OOBE_20161026.SetupRobot', { token: t3._id, id: 'other-robot-name-here' });
...
test('setupRobot errors: expired token 401 TOKEN_EXPIRED (not deleted), unknown 404', async () => {
  ...
  const r = await amz('OOBE_20161026.SetupRobot', { token: token._id, id: 'x-y-z-robot' });
  ...
  const unknown = await amz('OOBE_20161026.SetupRobot', { token: 'NoSuchTok', id: 'x-y-z-robot' });
  assert.equal(unknown.status, 404);
  assert.equal(unknown.body.__type, 'TOKEN_NOT_FOUND');
});

test('unsigned OOBE exceptions require the exact target; unknown op is 400', async () => {
  const store = getStore();
  const owner = store.accountByEmail('jane@jetson.test');
  ...
  assert.equal(weird.body.__type, 'MISSING_AUTH_HEADER');
...
test('getStatus flips to complete once the token is consumed (or invalid)', async () => {
  ...
  const owner = store.accountByEmail('jane@jetson.test');
  const t = mintSetupToken(store, owner._id);
  assert.equal((await amz('OOBE_20161026.GetStatus', { token: t._id })).body.complete, false);
  await amz('OOBE_20161026.SetupRobot', { token: t._id, id: 'status-test-robot-abc' });
  assert.equal((await amz('OOBE_20161026.GetStatus', { token: t._id })).body.complete, true);
  assert.equal((await amz('OOBE_20161026.GetStatus', { token: 'bogus' })).body.complete, true);
});

test('prepareRobot: authenticated with a verified SigV4 signature; reuses the live token', async () => {
  const store = getStore();
  const owner = store.accountByEmail('jane@jetson.test');
  const signed = () => signedLoopHeaders(store, base, 'OOBE_20161026.PrepareRobot', {}, owner.accessKeyId);

  const anon = await amz('OOBE_20161026.PrepareRobot', {});
  assert.equal(anon.status, 401);
  assert.equal(anon.body.__type, 'MISSING_AUTH_HEADER');

  const r1 = await amz('OOBE_20161026.PrepareRobot', {}, signed());
  ...
  const r2 = await amz('OOBE_20161026.PrepareRobot', {}, signed());
...
test('Loop.List: robot creds -> its one loop with `id` (what jibo-system-backup.js reads)', async () => {
  ...
  const token = mintSetupToken(store, owner._id);
  await amz('OOBE_20161026.SetupRobot', { token: token._id, id: 'rocket-maple-pixel-comet' });
