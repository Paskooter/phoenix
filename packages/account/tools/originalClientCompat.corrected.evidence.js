// Corrected A-02 review evidence: exercise the pinned original Jibo AWS client
// signer and Jetstream native sign-before-body construction through ephemeral
// Phoenix Classic + Account listeners. Run explicitly; this is intentionally
// outside ordinary unit-test discovery. The historical sibling retains the old
// ***hidden*** expectation; this file records the source-correct real account
// secret as a synthetic-secret hash only.
...
after(() => {
  if (process.env.PHX_A02_CORRECTED_OUT) {
    writeFileSync(process.env.PHX_A02_CORRECTED_OUT, `${JSON.stringify({
      generatedAt: new Date().toISOString(),
      runKind: 'corrected-original-sdk-compatibility',
      historicalArtifact: 'A-02-original-client-compat-20260905.json',
      syntheticOnly: true,
      sourcePins: {
...
test('source validation cases are captured with exact Phoenix response headers and bodies', async () => {
  const cases = [
    ['top-null', 'null'],
  ...
  for (const [name, body] of cases) {
    ...
    const response = await send(request);
    // Phoenix's JSON parser rejects a top-level JSON null before the source
    // Joi decorator sees it; retain that observed 400 as a compatibility
    // difference in the artifact. Object-shaped invalid payloads reach the
    // corrected 422 branch.
    const expectedStatus = name === 'top-null' ? 400 : 422;
    ...
    if (expectedStatus === 422) assert.equal(typeof response.body.message, 'string');
    observations.validation.push({ name, request: rawRequest(request), response: responseAssertion(response) });
  }
});
